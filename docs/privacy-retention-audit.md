...
> **Post-audit persistence acknowledgement status (2026-08-09):** On `fix/persistence-save-acknowledgement`, the current WIP makes onboarding completion and the audited user-triggered Saved flows wait for the exact resulting snapshot to be written through the active storage adapter. The checked scope is onboarding completion; Check-In creation and context/note updates; Protection setup, night setup, pause, resume, and turn-off; daily Reset completion; Pause completion including Save and Close; and Arousal Control completion plus draft/latest-log note saves. Saved, history, count, Progress, journey, and onboarding-result presentation read the durable projection rather than accepted-only state. A failed or timed-out write can leave an accepted mutation in session memory without navigating to or rendering durable success; its opaque token retries that exact revision without rerunning the domain transformation. Shared persistence feedback is revision-bound, so settlement from an obsolete retry cannot clear or replace the current mutation's feedback. Pause completion navigation carries the exact existing record ID and validates that ID in durable state; no-parameter entry is labeled as history and cannot use an older record while a newer accepted completion remains unconfirmed. Arousal completion likewise carries its exact completed-log ID and requires that exact valid log in durable state; no-parameter entry is labeled as history, and neither mode can substitute an older durable log while a newer accepted completion remains unconfirmed. Hydration watchdog expiry returns to explicit recoverable error even when a still-running deletion owns and blocks storage. Late deletion success invalidates hydration work queued during deletion before installing post-delete defaults, so its later settlement cannot replace a new mutation accepted after reset navigation unblocks; late failure returns to recoverable error without installing defaults, and the verified pre-removal failure path preserves the prior payload. These are application-level adapter acknowledgements, not claims of encryption, secure deletion, backup exclusion, operating-system flush guarantees, or platform-level durability. The original timing finding below remains evidence for the audited commit.
