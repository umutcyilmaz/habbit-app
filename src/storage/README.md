...
The six session APIs in `bloomMasturbationTransitions.ts`, re-exported from `bloomState.ts`, are `startMasturbationSessionState`, `startMasturbationPauseState`, `endMasturbationPauseState`, `endMasturbationSessionState`, `completeMasturbationSessionFeedbackState`, and `discardActiveMasturbationSessionState`. [DATA_MODEL.md](../../docs/DATA_MODEL.md#session-transitions) defines their exact input shapes and preconditions. Every identity/time is explicit, and invalid transitions are exact no-ops.

Only one unfinished session can occupy `currentSession`, including after reload. New starts require enabled Tracking, no unfinished session, and no effective Reset restriction at supplied `startedAt`; onboarding is not a direct prerequisite. Exactly at/after the current Reset attempt's 15-day boundary, persisted `active` status alone does not block a start, and starting never advances Reset. Active session/pause timers use timestamps rather than persisted ticking counters. Ending derives whole elapsed seconds, including pause time, closes any active pause at the same end time, and retains `awaiting_feedback` until valid feedback appends one completed history entry. No new start is allowed meanwhile. Feedback completion remains possible with Tracking disabled. Discard clears only an active physical session and creates no record/event; awaiting feedback cannot be discarded.

`bloomMasturbationTrackingTransitions.ts` adds `enableMasturbationTrackingState(state)` and `disableMasturbationTrackingState(state)`, re-exported from `bloomState.ts`. They change only `enabled`, preserving unfinished work, completed history, and all other slices without timestamps or toggle-history metadata. Manual enable explicitly allows Reset `inactive`, `recommended`, or `completed` and rejects `baseline_pending`, `active`, or `assessment_pending`, including elapsed-but-still-active Reset. Disable is allowed regardless of Reset lifecycle; existing actions can still resolve the preserved session. Already-matching values and invalid state are exact no-ops. Disabled Tracking with an unfinished session remains a valid persisted state.

The domain-only `getResetRestrictionStatus(resetJourney, at)` and `getMasturbationTrackingAvailability(state, at)` provide plain product-policy read models. Active Reset delegates elapsed progress to `getResetProgress`: before 15 days restriction is active; at/after 15 days it is over and `needsCompletionTransition` is true until an explicit transition runs. Non-active Reset returns all false flags with null progress. Availability exposes enabled/current-session status, shared restriction facts, and start permission, with block precedence `trackingDisabled`, `activeSession`, `awaitingFeedback`, `resetRestriction`, then null. Invalid canonical time or indeterminate policy returns null. [DATA_MODEL.md](../../docs/DATA_MODEL.md#product-policy-read-models) details the shapes. Reading policy never completes Reset, enables Tracking, or writes state; Home integration, routing, and provider/UI wiring remain deferred. Phase 1N changes no model, schema, persistence version/key, or migration.

The separate domain-only `getBloomHomeReadModel(state, at)` composes the five new product slices into `{ primaryAction, primaryTracker, secondaryTracker, trackingAvailability, urgeControlProgress, urgeControlAvailable: true }`. It reuses existing selectors, including Reset policy from Tracking availability; invalid required time/progress returns null. Priority is unfinished session work, active Urge Control, elapsed Reset completion, assessment, baseline, effectively active Reset, unaccepted stored onboarding recommendation, recommended Reset, then tracker action. Day-15 output requests explicit completion persistence without running it. Not-completed product onboarding alone forces no action, and stored recommendations are never rescored.

Enabled Tracking is primary, with active Content-Free secondary; when Tracking is disabled, active Content-Free becomes primary. Trackers remain represented under higher-priority lifecycle/resume actions. Output contains semantic IDs and plain facts only; no route, presentation copy, storage write, generated time/identity, or legacy plan dependency is added. [DATA_MODEL.md](../../docs/DATA_MODEL.md#new-product-home-read-model) gives the action union and payloads. Phase 1O leaves v7 models/schema/key/migrations unchanged and persists no Home priority, tracker role, or computed progress. The legacy `getNextBloomAction`, presentation layer, and Today remain untouched; navigation mapping, quick-action presentation, and provider/UI integration are deferred.

Explicit-content feedback uses `{ kind: "masturbationSession", sessionId }` and `session.endedAt` as the V1 Content-Free occurrence anchor; that timestamp does not claim the exact first content-use instant. Feedback `recordedAt` must be canonical and at or after session end. Inactive Content-Free, false explicit-content feedback, or a session ending before the current activation leaves Content-Free unchanged. When an active activation applies, completion requires an unused violation ID and event time at or after the current streak start. Existing recorded or undone session sources, conflicting IDs, contradictory activation timing, or unsafe backdating reject the entire feedback mutation.

An applicable feedback submission creates the completed session and one recorded Content-Free violation together, retains the original `streakBefore`, updates best whole-second duration, and restarts the streak at session end. Content-Free stays active with its activation and histories preserved. Other product/legacy slices remain unchanged; session feedback never creates a Reset violation. These snapshots use the existing coordinator without new timers, background writes, schema fields, version/key changes, or migrations. Awaiting-feedback deletion, historical replay, and UI remain deferred.

`bloomMasturbationCorrections.ts` adds `editCompletedMasturbationSessionFeedbackState(state, { sessionId, feedback, editedAt, contentFreeViolationId? })` and `deleteCompletedMasturbationSessionState(state, { sessionId, deletedAt })`, re-exported from `bloomState.ts`. They target exactly one completed history entry and validate supplied canonical correction time at or after its end. Full feedback replacement changes only erection quality, intentional explicit-content use, and ending reason; session identity, timestamps, duration, and pauses are immutable. Enabled Tracking is not required. Current/awaiting sessions and all unrelated state remain unchanged; identical feedback, invalid input, unsafe correction, and repeated deletion return the original state.

Explicit-content changes reconcile the session-derived Content-Free source atomically. False-to-true outside known activation periods changes feedback only; inside the current active streak it creates one supplied-ID violation anchored at the immutable session end, with `recordedAt = editedAt` and the exact original snapshot. A required past-activation insertion, ambiguous shared boundary, or event before the current streak is rejected. True-to-false safely restores the latest linked event's `streakBefore` and retains an undone tombstone. Its own undone link can later be safely reapplied only while the restored snapshot remains current and no later effective event requires replay: identity, anchor, original `recordedAt`, and snapshot are retained, `undoneAt` is removed, and `editedAt` must be at or after the prior undo. Quality/reason-only edits never mutate Content-Free.

Deletion removes the completed session without adding a session tombstone. An existing undone Content-Free link stays intact, including its original `undoneAt`; a recorded link must first be safely restored and tombstoned in the same transaction, with correction time at or after `recordedAt`. Unsafe later activity, changed activation, or historical recalculation blocks the entire correction. Any Reset violation/tombstone referencing the session blocks explicit-content edits and deletion, while simple quality/reason edits remain allowed. These actions never rewrite Reset or invoke standalone manual undo. [DATA_MODEL.md](../../docs/DATA_MODEL.md#completed-session-corrections) details the exact guards. Phase 1L adds no model/schema, persistence version/key, migration, or coordinator change; stored historical durations are not recalculated.

`bloomContentFreeTransitions.ts` adds four pure APIs re-exported from `bloomState.ts`: `activateContentFreeState(state, { activationId, activatedAt })`, `deactivateContentFreeState(state, { endedAt })`, `recordManualContentFreeViolationState(state, { violationId, logActionId, occurredAt, recordedAt })`, and `undoManualContentFreeViolationState(state, { violationId, undoneAt })`. Activation uses a fresh activation identity/time after past periods; deactivation archives the current activation once and updates best from the ended streak. Reactivation preserves all history. Neither action changes Reset or Tracking.

Manual logging requires active Content-Free, valid supplied identities/times, `recordedAt >= occurredAt`, and occurrence within the current effective streak. It constructs a manual source, snapshots the original streak, appends one recorded violation, updates best, and restarts the streak without deactivating. Recorded and undone IDs/sources remain consumed. Backdated events inside the current streak are supported; earlier insertion requires deferred replay. Effective Reset restriction at `occurredAt` blocks this standalone path so its atomic Reset violation action owns the event. At/after 15 days it may proceed under the other Content-Free guards even with persisted active Reset, which remains unchanged. The Reset violation path still refuses events at/after that boundary.

Standalone undo requires a latest safely reversible recorded manual event in the same active activation, unchanged post-event streak state, and canonical `undoneAt >= recordedAt`. It restores `streakBefore` and retains an undone tombstone. Session sources and manual sources present in Reset violation history are rejected; those events belong to their owning transaction. Later effective activity, changed activation, invalid input, and repeated undo are exact no-ops. Other product and legacy slices remain unchanged.

The domain-only `getContentFreeProgress(contentFree, now)` selector returns active current seconds/full days, effective best, and `hasEffectiveViolation`; inactive results omit current progress and retain historical best. Effective best includes the growing active streak, while only recorded events across activation history make the flag true. Invalid canonical time returns null and an early clock clamps current progress to zero. No counter, best value, or flag is persisted merely as time passes. [DATA_MODEL.md](../../docs/DATA_MODEL.md#content-free-progress) gives the exact result shape. Phase 1K uses existing v7 models/key/migrations; arbitrary replay and same-local-day collapse remain deferred.

`bloomUrgeControlTransitions.ts` provides ten pure actions, re-exported from `bloomState.ts`, for event start, interrupt completion, technique selection, phone-away start/end, outcome, trigger, optional second-line choice, event completion, and active discard. [DATA_MODEL.md](../../docs/DATA_MODEL.md#urge-control-transitions) defines the exact APIs and inputs. One event may be active; IDs/times are supplied, existing record IDs cannot be reused, and invalid or repeated completed steps are exact no-ops. No Tracking, Content-Free, Reset, or onboarding state gates the flow, and all other slices remain unchanged.

The guided order is interrupt → technique → phone-away start/end → outcome → trigger, with optional second-line choice after a non-reduced outcome. Technique may change before phone-away starts, while outcome and trigger may be corrected until completion. Changing outcome to `reduced` clears a prior second-line action. That action only records a choice; it sends no message and performs no device control or scheduling. Completion requires all guided facts, appends once, and clears the active slot. Discard clears only the active event without history/tombstones; completed Urge Control editing/deletion/undo remains deferred.

The domain-only `getUrgeControlProgress(urgeControl, now)` derives the resume stage and whole nonnegative elapsed event seconds. Optional phone-away seconds appear after its start and freeze at its end. It returns null without an active event or with invalid time. Actual timestamps carry no exact interrupt or two-minute constraint. Hydration preserves the active event without automatic completion/discard, and no ticking counter or background write is added. Existing v7 models, schema, key, migrations, and historical optional-step compatibility remain unchanged. New transitions enforce ordered prerequisites without silently repairing or inventing missing facts. No UI, navigation, Today/Home integration, notification, or legacy integration is added.
...
`getResetProgress(resetJourney, now)` derives active progress from `currentAttempt.startedAt` and supplied time, using full 24-hour durations clamped to 0–15 days. Users do not manually complete days. `isPeriodComplete` becomes true at 15 days even if persisted status remains `active`; neither this selector nor loading, validation, or hydration writes completion or best progress. An explicit transition persists the period's end. Historical attempts retain their ended progress, while active attempts reject `completedDays`. [DATA_MODEL.md](../../docs/DATA_MODEL.md#elapsed-progress) details selector output and clock behavior.

`completeElapsedResetPeriodState(state, { observedAt })` requires valid active Reset state and a canonical supplied observation time at or after 15 elapsed days. It moves Reset to `assessment_pending`, sets best progress to 15, and converts the same current attempt to completed with 15 days. Journey and attempt completion times equal `currentAttempt.startedAt + 15 * 24 hours`, even if observation happens later. The final attempt stays in `currentAttempt`; original journey start, baseline, previous attempts, and tombstones are preserved. All other slices retain their references. This action does not submit an assessment or enable Tracking, and invalid/early/repeated calls are exact no-ops.

`completePostResetAssessmentState(state, assessment)` requires `assessment_pending`, matching journey/attempt/baseline IDs, existing valid answer enums, a canonical submission time at or after the period end, and `masturbationTracking.currentSession === null`. It copies the assessment, completes Reset, and enables Tracking for `ready`, `notReady`, and `notSure` alike. Readiness never decides permission. The onboarding flow keeps Tracking disabled until submission; the behavioral restriction still ends at the 15-day boundary. Invalid input or unexpected unfinished sessions leave the entire state unchanged; completed assessments cannot be overwritten by retries.

Both transitions are defined in `bloomResetTransitions.ts` and re-exported from `bloomState.ts`. They preserve session history, Content-Free activation/streak/history, onboarding acceptance, Urge Control, and legacy state. Assessment time never changes the recorded Reset period end. Existing finished-state validation checks structural and temporal consistency without imposing new exact-duration arithmetic on historical records. Phase 1I uses existing v7 shapes and leaves the version, key, migrations, and persistence lifecycle unchanged. Those completion transitions add no report, post-Reset comparison, session, or recommendation logic.
...
Phase 1P adds [`createBloomProductAcknowledgedActions({ applyAcknowledgedMutation })`](../app/providers/bloomProductAcknowledgedActions.ts), with exported type `BloomProductAcknowledgedActions = ReturnType<typeof createBloomProductAcknowledgedActions>`. `BloomLocalStateProvider` memoizes the result and exposes one stable `productActions` group through `useBloomLocalState`. The groups cover onboarding, Reset, Tracking controls, sessions/corrections, Content-Free, and Urge Control; [ARCHITECTURE.md](../../docs/ARCHITECTURE.md#new-product-application-actions) lists the exact public methods. Legacy provider methods and their separate factory remain intact.

Every command returns `Promise<BloomPersistedMutationResult>` and delegates to its existing transition within `applyAcknowledgedMutation(currentState => ...)`. Business rules stay in pure transitions; the facade neither captures React state nor changes caller-supplied inputs, IDs, or timestamps. All commands use the existing acknowledged runtime, with no direct AsyncStorage/save call, separate queue, timer, or navigation result. Changed state is accepted immediately; success still requires the exact snapshot's durable write. The provider's accepted `state`, `durableState`, and existing retry API retain their semantics.

A domain no-op returns the existing non-persisted `invalidSession` rejection and triggers no write. Hydration/deletion guards retain `hydrationPending`, `stateUnavailable`, and `deletionInProgress`; the facade does not queue blocked commands or bypass those results. Persistence failures, retry tokens, supersession, and deletion invalidation remain owned by the existing runtime. Read selectors are not commands: Home, Reset restriction, Tracking availability, Content-Free progress, and Urge Control progress remain outside the facade and are not automatically evaluated by the provider. Phase 1P changes no v7 model/schema/key/migration and adds no UI, Today, navigation, or route mapping.

## Product Flow Integration

Phase 1Q adds [`createBloomProductFlowActions({ productActions, now?, createId? })`](../app/flows/bloomProductFlowActions.ts), with `BloomProductFlowActions` inferred from its return type. It mirrors the grouped product command API, preparing IDs/current timestamps only for commands that need them and retaining direct references for the others. Prepared calls capture the operation time once; injected `createId(prefix, operationTime)` uses that same Date, with the default following the existing prefix/timestamp/random-nonce convention. Semantic inputs and existing target/reference IDs remain caller-supplied. A supplied violation occurrence time is preserved; otherwise occurrence equals the operation time.

Possible linked IDs are allocated before delegation without inspecting state: Reset/Content-Free activation IDs for acceptance, Reset violation/replacement/linked-Content-Free IDs for violation logging, and linked Content-Free IDs for session feedback/edits. Explicit manual Reset sources receive a generated log-action ID; supplied session sources retain their existing session ID. The authoritative transition decides whether to use the possible linked IDs. The flow layer calls only `productActions`, preserves `Promise<BloomPersistedMutationResult>` outcomes, and adds no validation policy, reconciliation, direct persistence, or selectors. Retry through the existing `retryPersistedMutation(retryToken)` API persists the already-accepted snapshot without replaying the flow or regenerating IDs/times. Provider, facade, domain transitions, runtime, v7 schema/key/migrations, and legacy behavior remain unchanged.
...
Unacknowledged mutations requested during loading are queued and applied to the validated state after hydration. Acknowledged commands are rejected by the runtime during loading or unavailable state. Unacknowledged mutations are ignored after a hydration error. Initial defaults are not autosaved, and canonical loaded state is not rewritten unless normalization or a real mutation requires it.
...
This command also runs the focused product-state, onboarding, plan-acceptance, Reset-baseline, Reset-violation, Reset-violation-undo, Reset-completion/assessment, Masturbation Session/Content-Free, completed-session correction, standalone Content-Free, Urge Control, product-policy/Tracking-control, and new Home core/composition suites. Reset coverage includes supplied baseline validation, history preservation, repeated-start no-ops, 24-hour progress boundaries, clock skew, older-schema migration, and no automatic completion. Violation coverage checks all reasons, atomic Content-Free effects, source/ID deduplication, and the 15-day boundary. Undo coverage includes sequential restoration, tombstones, stale retries, rollback metadata, ambiguous-history rejection, atomic snapshot safety, migration durability, and v7 round trips. Completion coverage checks the exact elapsed boundary on late observation, preserved histories, explicit assessment linkage/time, all readiness values enabling Tracking, unfinished-session rejection, idempotency, and both finished-state round trips. Session coverage checks start permissions, optional pauses, whole-second durations, persisted awaiting feedback, atomic Content-Free linkage/backdating/deduplication, active-only discard, malformed records, and v7 round trips. Correction coverage includes feedback edits, immutable session timing/pauses, atomic Content-Free undo/reapplication, deletion, Reset-link guards, historical replay rejection, idempotency, and v7 round trips. Standalone Content-Free coverage adds activation cycles, preserved best/history, manual backdating and Reset blocking, safe undo/source ownership, tombstones, dynamic progress, and active/inactive round trips. Urge Control coverage checks the complete ordered lifecycle, enums and in-progress corrections, optional second-line choices, completion/discard idempotency, feature independence, resumed stages, elapsed-time and clock safety, older partial-step compatibility, duplicate IDs, and active/completed v7 round trips. Product-policy and Tracking-control coverage checks explicit enable/disable lifecycle rules, unfinished-work preservation/resolution, deterministic block precedence, invalid clocks, current-attempt Day-15 boundaries, event-time session/Content-Free guards, no automatic Reset advancement, and unchanged v7 shapes. Home coverage checks the semantic action union and precedence combinations, tracker ordering independent of recommendations/legacy plans, Day-15 completion priority, Urge resume stages, stored recommendation handling, selector composition, invalid-time results, determinism, immutability, legacy isolation, and matching derived output after v7 round trips.

The same persistence command runs [`verify-bloom-product-actions.ts`](../../scripts/verify-bloom-product-actions.ts) and [`verify-bloom-product-actions-runtime.ts`](../../scripts/verify-bloom-product-actions-runtime.ts). These verify every grouped product command's delegation and acknowledged return, unchanged caller inputs, sequential use of current runtime state, no-op behavior, provider exposure/memoization and legacy coexistence, plus real runtime acknowledgement, failure/retry, hydration/deletion guards, and unchanged v7 persistence.

The persistence command also includes [`verify-bloom-product-flow-actions.ts`](../../scripts/verify-bloom-product-flow-actions.ts) and [`verify-bloom-home-flow-intents.ts`](../../scripts/verify-bloom-home-flow-intents.ts). These cover deterministic one-time ID/time preparation, exact command inputs and semantic-value preservation, linked IDs without policy duplication, unchanged acknowledgements, retry without flow replay, exhaustive/pure Home mapping with preserved payloads, and unchanged application/persistence boundaries.
