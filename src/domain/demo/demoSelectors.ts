...
export function selectTodayDashboardData(state: DemoAppState): TodayDashboardData {
  ...
  return {
    ...
    recommendationBody: `Evenings between ${window.startTime} and ${window.endTime} appear often in demo activity.`,
...
export function selectProgressSummary(state: DemoAppState): readonly ProgressMetric[] {
  return [
    ...
    },
    {
      ...
      detail: "A suggested evening support window is active in demo state."
    }
  ];
}
