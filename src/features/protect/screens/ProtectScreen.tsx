...
import {
  selectIsProtectionOff,
  selectIsProtectionPaused,
...
import { AppStateCard } from "../../../shared/components/states";
...
export function ProtectScreen() {
  ...
  const isPaused = selectIsProtectionPaused(state);
  const isOff = selectIsProtectionOff(state);
    ...
    <AppScreen>
      ...
      <View style={styles.stack}>
        {isPaused ? (
          <AppStateCard
            variant="empty"
            title="Support paused for tonight"
            body="No pressure. Your plan can continue tomorrow."
            action={{
              label: "Turn back on",
              onPress: turnSupportOn
            }}
          />
        ) : null}
        ...
        {isOff ? (
          <AppStateCard
            variant="comingNext"
            title="Sensitive Window Support is off"
            body="You can turn it back on anytime."
            action={{
              label: "Turn Support On",
              onPress: turnSupportOn
            }}
          />
        ) : null}
        ...
        <AppCard>
          <View style={styles.cardStack}>
            ...
            {supportLevels.map((level) => (
              <AppButton
                key={level.id}
                variant={protection.level === level.id ? "primary" : "secondary"}
                onPress={() =>
                  dispatch({
        ...
        <AppCard>
          <View style={styles.cardStack}>
            ...
            <View style={styles.actions}>
              <AppButton onPress={turnSupportOn}>Save support window</AppButton>
