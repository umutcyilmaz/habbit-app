...
`navigateBloomProductFlow(router, intent, method = "push")` is the thin execution boundary. It returns `false` without navigation for `featurePending` destinations. This slice's three destinations are `ready`; Phase 1T also makes [Content-Free](../content-free/README.md) ready and Phase 1U adds the [core Reset routes](../reset/PRODUCT_RESET.md). Urge Control and recommendation destinations remain deferred. There is no automatic navigation effect based on product state.
