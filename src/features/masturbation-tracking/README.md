...
`navigateBloomProductFlow(router, intent, method = "push")` is the thin execution boundary. It returns `false` without navigation for `featurePending` destinations. Only this slice's three destinations are `ready`; Urge Control, Reset, Content-Free, and recommendation destinations remain deferred. There is no automatic navigation effect based on product state.
