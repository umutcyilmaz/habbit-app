...
const outputDirectory = mkdtempSync(join(tmpdir(), "bloom-quiz-verification-"));
...
function runCommand(command, arguments_) {
  ...
  if (result.error) {
    console.error("Bloom quiz verification could not start.");
    return 1;
  }
...
function runVerification() {
  ...
  return runCommand(process.execPath, [
    join(outputDirectory, "scripts", "verify-bloom-quiz.js")
  ]);
}
