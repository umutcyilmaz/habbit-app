...
const outputDirectory = mkdtempSync(
  join(tmpdir(), "bloom-guided-flows-verification-")
);
...
function runCommand(command, arguments_) {
  ...
  if (result.error) {
    console.error("Bloom guided-flow verification could not start.");
    return 1;
  }
...
function runVerification() {
  ...
  const compileStatus = runCommand(process.execPath, [
    typescriptCompiler,
    "scripts/verify-bloom-guided-flows.ts",
    "--outDir",
    outputDirectory,
    "--module",
    "commonjs",
  ...
  return runCommand(process.execPath, [
    join(outputDirectory, "scripts", "verify-bloom-guided-flows.js")
  ]);
}
